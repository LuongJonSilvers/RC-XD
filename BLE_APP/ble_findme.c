    ...
    switch(event)
    {
        ...
        case CY_BLE_EVT_GATTS_WRITE_CMD_REQ:
		{
			...
			if( CY_BLE_LEDS_USR_LED_CHAR_HANDLE == write_req_param->handleValPair.attrHandle)
			{
                ...
                long val = write_req_param->handleValPair.value.val[0];
				printf("[INFO] : GATT write USR_LED characteristic with value decimal: %d\r\n", val);
                printf("[INFO] : GATT write USR_LED characteristic with value float: %f\r\n", val);
                printf("[INFO] : GATT write USR_LED characteristic with value hex: %x\r\n", val);
                motor_pwm_set(PWM_FREQUENCY, 56, true, true, false);
